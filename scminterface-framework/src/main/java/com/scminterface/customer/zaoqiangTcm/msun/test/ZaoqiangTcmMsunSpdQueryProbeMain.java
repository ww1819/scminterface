...
/**
 * 枣强中医院 SPD 查询接口联调入口（接口文档2）。
 * <p>依次调用 2.5.44 / 58 / 62 / 63 / 43 / 102，控制台打印 HIS 回参，不落库。</p>
 */
public class ZaoqiangTcmMsunSpdQueryProbeMain
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        printEnvBanner(env);
        MsunSignedHttpClient client = MsunOpenApiSupport.createClient(env);
        ...
        String dictRaw = invokeGet(env, client, ZaoqiangTcmMsunSpdApiPaths.DRUG_DICT_INFOS, dictParams);
        ...
        String catRaw = invokeGet(env, client, ZaoqiangTcmMsunSpdApiPaths.DICT_CATEGORY, catParams);
        ...
        String supRaw = invokeGet(env, client, ZaoqiangTcmMsunSpdApiPaths.DRUG_SUPPLIERES, supParams);
        ...
        String prodRaw = invokeGet(env, client, ZaoqiangTcmMsunSpdApiPaths.DRUG_PRODUCERES, prodParams);
        ZaoqiangTcmMsunOpenApiRunner.printResponse("2.5.63 生产厂商 /v1/drug-produceres", prodRaw);

        // 2.5.43（依赖字典与科室；失败时仅提示）
        ...
        String ykRaw = invokePost(env, client, ZaoqiangTcmMsunSpdApiPaths.QUERY_YK_INSTOCK, ykBody);
            ...
            String dictRaw) throws Exception
    {
        ...
        String stockRaw = invokeGet(env, client, ZaoqiangTcmMsunSpdApiPaths.DRUG_BATCH_STOCKS, stockParams);
    ...
    {
        System.out.println("租户: zaoqiang-tcm-001 | SPD查询联调 | 环境: " + env.getCode() + " (" + env.getLabel() + ")");
        System.out.println("baseUrl: " + env.getBaseUrl());
        System.out.println();
    }

    /** 解析 HIS 响应体，避免主流程依赖 fastjson 细节。 */
