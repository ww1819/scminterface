 ...
 */
@Component
public class ScmScheduledTask
{
    private static final Logger log = LoggerFactory.getLogger(ScmScheduledTask.class);
    ...
    public void execute()
    {
        try
        {
            ...
            {
                log.info("SCM定时任务已达到最大执行次数限制: {}/{}", currentExecCount, maxExecCount);
                return;
            }
            ...
            // 增加执行次数
            scheduledTaskService.incrementScmExecCount();
        }
        catch (Exception e)
        {
            log.error("SCM定时任务执行异常", e);
        }
    }
}
