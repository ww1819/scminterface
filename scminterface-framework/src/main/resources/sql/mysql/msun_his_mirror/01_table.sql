-- =============================================================================
-- 众阳云健康 HIS 镜像表 DDL（SPD 业务库，如 aspt）— 由 MsunHisMirrorSchemaService 自动加载
-- =============================================================================
-- 【非标准对象】众阳云健康（msun）专用；表名 m_msun_*（见 MsunHisMirrorTableNames）。
-- 默认 auto-schema=true：首次接口/探针/落库时按表 CREATE IF NOT EXISTS，无需手工执行。
-- auto-schema=false 或离线环境：USE 目标库后按「/」分段手工执行；清理见 99_drop_mirror_tables_optional.sql。
-- SPD 业务表字段见 spd/.../material/column.sql（非本脚本）。
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS `m_msun_merge_stock` (
  ...
  `hospital_id` VARCHAR(64) DEFAULT NULL COMMENT '医院ID（2.5.82 回参 hospitalId）',
  `org_id` VARCHAR(64) DEFAULT NULL COMMENT '机构ID（2.5.82 回参 orgId）',
  PRIMARY KEY (`mirror_id`),
...
-- 2.5.43 药房批次库存（列名与 API camelCase→snake_case 一致，见 MsunHisMirrorRowSupport）
CREATE TABLE IF NOT EXISTS `m_msun_drug_batch_stock` (
  ...
  `insert_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '插入时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `dept_id` VARCHAR(64) DEFAULT NULL COMMENT '药房科室ID（enrich 自 yc_dept_id）',
  `yc_dept_id` VARCHAR(64) DEFAULT NULL COMMENT '2.5.43 ycDeptId',
  `yc_dept_name` VARCHAR(200) DEFAULT NULL COMMENT '2.5.43 ycDeptName',
  `drug_id` VARCHAR(64) DEFAULT NULL COMMENT '药品/材料ID',
  `drug_spec_packing_id` VARCHAR(64) DEFAULT NULL COMMENT '规格包装ID',
  `drug_code` VARCHAR(128) DEFAULT NULL COMMENT '2.5.43 drugCode',
  `drug_name` VARCHAR(500) DEFAULT NULL COMMENT '2.5.43 drugName',
  `spec` VARCHAR(500) DEFAULT NULL COMMENT '2.5.43 spec 包装规格',
  `batch_number` VARCHAR(128) DEFAULT NULL COMMENT '批号（enrich 自 yc_batch_no，供唯一键）',
  `yc_batch_no` VARCHAR(128) DEFAULT NULL COMMENT '2.5.43 ycBatchNo',
  `stock_id` VARCHAR(64) DEFAULT NULL COMMENT '库存ID（遗留列）',
  ...
  `quantity` DECIMAL(18,4) DEFAULT NULL COMMENT '库存数量（enrich 自 stock_amount）',
  `stock_amount` DECIMAL(18,4) DEFAULT NULL COMMENT '2.5.43 stockAmount',
  `buy_price` DECIMAL(18,4) DEFAULT NULL COMMENT '进价',
  `retail_price` DECIMAL(18,4) DEFAULT NULL COMMENT '零售价',
  `produce_date` VARCHAR(32) DEFAULT NULL COMMENT '2.5.43 produceDate',
  `effective` VARCHAR(32) DEFAULT NULL COMMENT '有效期（enrich 自 effective_date）',
  `effective_date` VARCHAR(32) DEFAULT NULL COMMENT '2.5.43 effectiveDate',
  `drug_catagory_id` VARCHAR(64) DEFAULT NULL COMMENT '2.5.43 drugCatagoryId',
  `catagory_name` VARCHAR(200) DEFAULT NULL COMMENT '2.5.43 catagoryName',
  `producer_id` VARCHAR(64) DEFAULT NULL COMMENT '生产厂商ID',
  `producer_name` VARCHAR(500) DEFAULT NULL COMMENT '生产厂商（enrich 自 producer_cnname）',
  `producer_cnname` VARCHAR(500) DEFAULT NULL COMMENT '2.5.43 producerCnname',
  `supplier_id` VARCHAR(64) DEFAULT NULL COMMENT '供应商ID',
  `supplier_name` VARCHAR(500) DEFAULT NULL COMMENT '供应商',
  `packing_id` VARCHAR(64) DEFAULT NULL COMMENT '包装ID',
  `packing_name` VARCHAR(64) DEFAULT NULL COMMENT '2.5.43 packingName',
  `min_packing_name` VARCHAR(64) DEFAULT NULL COMMENT '2.5.43 minPackingName',
  `hospital_id` VARCHAR(64) DEFAULT NULL COMMENT '医院ID',
  `org_id` VARCHAR(64) DEFAULT NULL COMMENT '机构ID',
  PRIMARY KEY (`mirror_id`),
  ...
  KEY `idx_tenant_id` (`tenant_id`),
  KEY `idx_drug_id` (`drug_id`),
  KEY `idx_pharmacy_stock` (`pharmacy_stock_id`),
  KEY `idx_yc_stock_query` (`yc_stock_query_id`),
  KEY `idx_sync_batch` (`sync_batch_no`)
