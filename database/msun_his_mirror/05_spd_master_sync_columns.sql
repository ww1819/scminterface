...
-- 耗材：HIS 规格包装ID（对齐 m_drug_dict.drug_spec_packing_id）
ALTER TABLE `fd_material`
  ADD COLUMN `his_spec_packing_id` VARCHAR(64) DEFAULT NULL COMMENT 'HIS规格包装ID（众阳 drug_spec_packing_id）' AFTER `his_id`;
/
...
-- 计量单位：HIS 单位ID（众阳 min_packing_id / dose_unit_id）
ALTER TABLE `fd_unit`
  ADD COLUMN `his_unit_id` VARCHAR(64) DEFAULT NULL COMMENT 'HIS计量单位ID' AFTER `unit_name`;
/
